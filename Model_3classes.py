...
PatientsIdTrain,PatientsIdVal, PatientsIdTest = SelectPatientsTrainVal(base_dir,.1,.3,.55)  #La partie validation correspond à 10% de notre dataset.
...
X_val = []
Y_val = []
X_val, Y_val = ExtractXY(base_dir, PatientsRead=PatientsIdVal, image_size=256, num_classes=3, ShowProgress=True)

X_train = []
Y_train = []
X_train, Y_train = ExtractXY(base_dir, PatientsRead=PatientsIdTrain, image_size=256, num_classes=3, ShowProgress=True)
...
def unet():
	...
	conv10 = Conv2D(3, 1, activation='softmax')(conv9) #changer ici pour 2 ou classes 
...
def means_iou(y_true, y_pred):
    y_true = K.argmax(y_true, axis=-1)
    y_pred = K.argmax(y_pred, axis=-1)
    y_true = K.cast(y_true, 'int32')
    y_pred = K.cast(y_pred, 'int32')

    # Compute the IoU mean
    ious = []
    for i in range(K.int_shape(y_pred)[-1]):
        true_idx = K.equal(y_true, i)
        pred_idx = K.equal(y_pred, i)
        intersection = K.sum(K.cast(true_idx & pred_idx, 'float32'))
        union = K.sum(K.cast(true_idx | pred_idx, 'float32'))
        iou = intersection / union
        ious.append(iou)
    return K.mean(K.stack(ious), axis=0)

autoencoder.compile(optimizer='adam', loss='categorical_crossentropy', metrics=[tf.keras.metrics.MeanIoU(num_classes=3)]) #tf.keras.metrics.MeanIoU(num_classes=3), 
#dice_loss 
...
filepath="Model25_weights_best.hdf5"
checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True)
es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=3)
callbacks_list = [checkpoint, es]


history = autoencoder.fit(X_train,Y_train_cat,batch_size=16,epochs = 10,callbacks=callbacks_list, verbose=1,shuffle=True,validation_data=(X_val,Y_val_cat)) #changer les Y train et val cat
...
"""
plt.figure()
plt.plot(history.history['means_iou'])
plt.plot(history.history['val_means_iou'])
plt.title('model IoU')
plt.ylabel('Score IoU')
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper left')
plt.savefig('./Results/Model21_IoU.png')
plt.show()
"""
...
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper left')
plt.savefig('./Results/Model25_IoU.png')
plt.show()
...
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper left')
plt.savefig('./Results/Model25_Loss.png')

autoencoder.save('./Results/Model25')
